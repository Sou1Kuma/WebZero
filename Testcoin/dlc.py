...
def read_and_decrypt(file_path, skin_key=None, keys_file=None):
    with open(file_path, 'rb') as file:
        ...
        uuid = file.read(uuid_length).decode('utf-8')
        if magic != 0x9BCFB9FC:
...
def compress_files_zip(source_folders, dlc_pack_name, output_folder, is_addon=False):
    print("Compressing files...", end="", flush=True)
    
    is_mobile = hasattr(sys, 'getandroidapilevel') if 'sys' in globals() else False
    ...
    # --- NEW LOGIC: ถ้าเป็นโฟลเดอร์เดียว ให้ zip โฟลเดอร์ root แล้วเปลี่ยนนามสกุล ---
    if not is_addon and isinstance(source_folders, (list, tuple)) and len(source_folders) == 1:
        folder_to_zip = source_folders[0]
        out_path = f"{base_name}{extension}"
        zip_temp = f"{base_name}.zip"
        import shutil
        print(f"DEBUG: base_name={base_name}, extension={extension}, out_path={out_path}")
        shutil.make_archive(base_name, 'zip', folder_to_zip)
        # ลบไฟล์ output เดิมถ้ามี
        if os.path.exists(out_path):
            os.remove(out_path)
        # เปลี่ยนชื่อ .zip เป็น .mcpack/.mctemplate
        if os.path.exists(zip_temp):
            os.rename(zip_temp, out_path)
        print("Zipped root folder as:", os.path.basename(out_path))
        return out_path
    # --- END NEW LOGIC ---
                    ...
                    for root, _, files in os.walk(folder):
                        for file in files:
                            if file not in ["signatures.json", "contents.json"]:
                                ...
                                arcname = os.path.join(folder_type, os.path.relpath(filepath, folder))
        ...
        def process_file(args):
            ...
            try:
                ...
                with open(filepath, 'rb') as f:
                    buffer.write(f.read())
                buffer.seek(0)
                return arcname, buffer.getvalue()
            except Exception as e:
                ...
                for root, _, files in os.walk(folder):
                    for file in files:
                        if file not in ["signatures.json", "contents.json"]:
                            filepath = os.path.join(root, file)
                            arcname = os.path.join(folder_type, os.path.relpath(filepath, folder))
                            files_to_process.append((filepath, arcname))
        else:
            source_folder = source_folders if isinstance(source_folders, str) else source_folders[0]
            for root, _, files in os.walk(source_folder):
                for file in files:
    ...
    print("\r" + " " * len("Compressing files...") + "\r", end="", flush=True)
...
def get_skin_pack_name(extracted_folder_path):
    ...
    with open(lang_file_path, 'rb') as lang_file:
        ...
        try:
            ...
            decoded_line = first_line.decode('utf-8', errors='ignore')
